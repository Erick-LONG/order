...
class PayService():
        ...
        try:
            ...
            for item in items:
                ...
                if int(item['number']) > int(tmp_left_stock):
                    raise Exception('您购买的太火爆了，剩余：%s，您购买的数量是：%s'%(tmp_left_stock,item['number']))
                tmp_ret = Food.query.filter_by(id=item['id']).update({
                    "stock":int(tmp_left_stock) - int(item['number'])
                })

                if not tmp_ret:
                    raise Exception('下单失败，请重新下单1')
    ...
    def orderSuccess(self,pay_order_id=0,params=None):
        try:
            ...
            pay_order_info.pa_time = getCurrentDate()
    ...
    def geneOrderSn(self):
        ...
        sn = None
        while True:
            str = "%s-%s"%(int(round(time.time()*1000)),random.randint(0,999999999))
